...
def is_validation(check_list):
    if check_list['blood_group'] not in blood_types:
        return False,['blood_group']
    elif check_list['company'] not in black_list:
        return False, ['company']
    elif '@' not in check_list['email']:
        return False ['email']
    elif len(check_list['name']) < 2 or len(check_list['name']) > 30 :
        return False['name']
